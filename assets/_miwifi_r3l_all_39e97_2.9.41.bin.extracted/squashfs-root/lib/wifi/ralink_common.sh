...
enable_ralink_wifi() {
	...
	if [ -f /lib/wifi/singlesku/"$country"_SingleSKU.dat ];then
		ln -s /lib/wifi/singlesku/"$country"_SingleSKU.dat /etc/Wireless/"$device"/SingleSKU.dat
	else
		ln -s /lib/wifi/singlesku/CN_SingleSKU.dat /etc/Wireless/"$device"/SingleSKU.dat
	fi
	[ "$disabled" == "1" ] && return
	for vif in $vifs; do
