...
-- HBase
--
local logServerDomain = luci.util.trim(luci.util.exec("uci get /etc/config/miwifi.server.LOG"))
local HBASE_LOG_UPLOAD_URL = "https://"..logServerDomain.."/xiaoqiang_log/"
local HBASE_CONFIG_UPLOAD_URL = "https://"..logServerDomain.."/xiaoqiang_config/"
...
local FILE_PATH = XQConfigs.LOG_ZIP_FILEPATH
local FILE_TYPE = "tar.gz"
local LOG_URL = "https://"..logServerDomain.."/log/lite/common/%s"
local LOG_CMD = "curl -k -i -X POST -F 'id=%s' -F '_n=%s' -F '_t=%s' -F 'extra_data={\"version\":\"%s\", \"sn\":\"%s\", \"suffix\":\""..FILE_TYPE.."\"}' -F 'mode=%s' -F 'key=%s' -F 'payload=@"..FILE_PATH.."' %s"
...
-- XiaoQiang
--
local apiServerDomain = luci.util.trim(luci.util.exec(XQConfigs.SERVER_CONFIG_ONLINE_URL))
local XIAOQIANG_SERVER = "http://"..apiServerDomain
if XQConfigs.SERVER_CONFIG == 1 then
...
--     local XQSysUtil = require("xiaoqiang.util.XQSysUtil")
--     local param = {}
--     local d = XQFunction.mattool_identify_device()
