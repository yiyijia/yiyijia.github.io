...
function setWifiBasicInfo(wifiIndex, ssid, password, encryption, channel, txpwr, hidden, on, bandwidth, bsd, txbf)
    local network = LuciNetwork.init()
    local wifiNet = network:get_wifinet(_wifiNameForIndex(wifiIndex))
    local wifiDev = network:get_wifidev(LuciUtil.split(_wifiNameForIndex(wifiIndex),".")[1])
    if wifiNet == nil then
...
--for R3/R3P/R1C/R3D/R1D/R2D
function apcli_set_connect(apcliitem)
    local cmdssid =  apcliitem.cmdssid
    local ifname = apcliitem.ifname
    local cmdencryption = XQFunction._cmdformat(apcliitem.encryption)
    local cmdpassword = apcliitem.cmdpassword

    if HARDWARE:match("^r3d") then
        local is_connected = LuciUtil.exec("/usr/sbin/check_apcli_connected "..cmdssid.." "..ifname.." "..apcliitem.enctype.." "..cmdencryption.." "..cmdpassword.."")
        --logger.log(4, string.format("is_connected=%d", is_connected))
        ...
        if apcliitem.enctype:match("AES") then
            os.execute("killall -9 wpa_supplicant")
            --os.execute("rm -rf /var/run/wpa_supplicant-global.pid")
            os.execute("echo \'ctrl_interface=/var/run/wpa_supplicant-"..ifname.."\' > /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'network={\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	scan_ssid=1\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	ssid=\""..cmdssid.."\"\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	key_mgmt=WPA-PSK\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            if cmdencryption:match("WPA2PSK") then
                os.execute("echo \'	proto=RSN\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            else
                os.execute("echo \'	proto=WPA\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            end
            os.execute("echo \'	psk=\""..cmdpassword.."\"\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	pairwise=CCMP\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	group=CCMP TKIP\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'}\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("wpa_supplicant -i "..ifname.." -Dathr -c /var/run/wpa_supplicant-"..ifname..".conf -B")
            --os.execute("wpa_supplicant -i "..ifname.." -Dathr -c /var/run/wpa_supplicant-"..ifname..".conf -b br-lan &")
            --os.execute("wpa_supplicant -g /var/run/wpa_supplicantglobal -B -P /var/run/wpa_supplicant-global.pid")
            --os.execute("[ -f \"/var/run/wpa_supplicant-"..ifname..".lock\" ] && rm /var/run/wpa_supplicant-"..ifname..".lock")
            --os.execute("wpa_cli -g /var/run/wpa_supplicantglobal interface_add  "..ifname.." /var/run/wpa_supplicant-"..ifname..".conf athr /var/run/wpa_supplicant-"..ifname.."")
            --os.execute("touch /var/run/wpa_supplicant-"..ifname..".lock")
        elseif apcliitem.enctype:match("TKIP") then
            ...
            os.execute("echo \'	psk=\""..cmdpassword.."\"\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	pairwise=TKIP\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	group=CCMP TKIP\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'}\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("wpa_supplicant -i "..ifname.." -Dathr -c /var/run/wpa_supplicant-"..ifname..".conf -B")
        elseif apcliitem.enctype:match("WEP") then
            ...
            os.execute("echo \'	scan_ssid=1\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	ssid=\""..cmdssid.."\"\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	key_mgmt=NONE\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	wep_key0="..cmdpassword.."\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	wep_tx_keyidx=0\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'	auth_alg=OPEN\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("echo \'}\' >> /var/run/wpa_supplicant-"..ifname..".conf")
            os.execute("wpa_supplicant -i "..ifname.." -Dathr -c /var/run/wpa_supplicant-"..ifname..".conf -B")
...
-- miwifi ready
                 elseif idx_new.mode == "ap" and idx_new.network == "ready" then
                     uci_new:set("wireless", iface_new['.name'], "ssid", "miwifi_ready")
    ...
    if not XQFunction.isStrNil(mac) and not XQFunction.isStrNil(name) then
        ...
        elseif tonumber(model) == 1 then
            if HARDWARE:match("^r3d") then
                ...
                -- os.execute("iwpriv wl14 maccmd_sec 1")
            else
                os.execute("wl -i wl0 mac \""..macstr.."\"")
                os.execute("wl -i wl1 mac \""..macstr.."\"")
                -- os.execute("wl -i wl1.2 mac \""..macstr.."\"")
...
elseif HARDWARE:match("^r1c") or HARDWARE:match("^r3") then
