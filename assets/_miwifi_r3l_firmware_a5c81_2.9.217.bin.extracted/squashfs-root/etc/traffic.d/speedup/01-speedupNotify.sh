...
if [ "$SPEEDUP_DIR"x = "down"x ]; then
	/usr/sbin/datacenterClient -h localhost -p 9090 -i '{"api":634,"pluginID":"2882303761517410304","info":"{\"api\":1009}"}' >/dev/null 2>&1
elif [ "$SPEEDUP_DIR"x = "up"x ]; then
        #logger -t EbitSPEEDUP -p9 "upupup"
	/usr/sbin/datacenterClient -h localhost -p 9090 -i '{"api":634,"pluginID":"2882303761517545233","info":"{\"api\":1009}"}' >/dev/null 2>&1
else
