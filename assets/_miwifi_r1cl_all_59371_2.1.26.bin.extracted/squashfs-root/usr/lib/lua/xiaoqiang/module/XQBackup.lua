...
local function _mi_wifi_info_restore(info)
    ...
    if info then
        ...
        local w2 = info["5g"]
        if w1 then
            wifi.setWifiBasicInfo(1, w1.ssid, w1.password, w1.encryption, w1.channel, w1.txpwr, w1.hidden, w1.on, w1.bandwidth)
        end
        if w2 then
            wifi.setWifiBasicInfo(2, w2.ssid, w2.password, w2.encryption, w2.channel, w2.txpwr, w2.hidden, w2.on, w2.bandwidth)
        end
    end
end
...
local MESSAGES = {
    ["mi_basic_info"]   = "路由器名和管理密码",
    ["mi_wifi_info"]    = "wifi设置(wifi名称和密码等)",
    ["mi_network_info"] = "上网设置(拨号方式和宽带帐号密码等)",
    ["mi_lan_info"]     = "DHCP服务和局域网IP设置",
    ["mi_arn_info"]     = "无线访问黑白名单",
    ["mi_access_info"]  = "设备访问权限"
}
