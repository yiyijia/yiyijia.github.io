...
local apclient_2g = UCI:get("misc", "wireless", "apclient_2G") or ""
local apclient_5g = UCI:get("misc", "wireless", "apclient_5G") or ""
...
function get_other_apcli(ifname)
   if not XQFunction.isStrNil(ifname) and ifname == apclient_5g then
       return apclient_2g
   elseif not XQFunction.isStrNil(ifname) and ifname == apclient_2g then
       return apclient_5g
   else
      return nil
   end
...
function get_apcli_ifname(band)
   if band == "5g" then
      return apclient_5g
   else
      return apclient_2g
...
function get_apcli_dev(ifname)
   if ifname == apclient_5g then
      return WIFI5G
   else
      return WIFI2G
   end
...
function getWifiBridgedClient(ifname)
    local LuciNetwork = require("luci.model.network").init()
    local client = {}
    local clientId = wifiStaClientId(ifname)
    if clientId then
...
function wifiStaClientId(ifname)
    local LuciNetwork = require("luci.model.network").init()
    local clientId

    if XQFunction.isStrNil(ifname) then
        return nil
    end
    local wifiDev = LuciNetwork:get_wifidev(get_apcli_dev(ifname))
    for _, wifiNet in ipairs(wifiDev:get_wifinets()) do
        if wifiNet:get("ifname") == ifname then
            clientId = wifiNet:id()
        end
    end
    return clientId
...
function deleteWifiBridgedClient(ifname)
    local LuciNetwork = require("luci.model.network").init()
...
function setWifiBridgedClient(ifname, ssid,encryption,enctype,key)
    local LuciNetwork = require("luci.model.network").init()
    local XQLanWanUtil = require("xiaoqiang.util.XQLanWanUtil")
    -- Set wifi
    local clientId = wifiStaClientId(ifname)
...
--防止环路
    local ifname_other,wifidev_other
    ifname_other = get_other_apcli(ifname)
    if not XQFunction.isStrNil(ifname_other) then
        local wifidev_other = LuciNetwork:get_wifidev(get_apcli_dev(ifname_other))
        local clientid_other = wifiStaClientId(ifname_other)
        if not XQFunction.isStrNil(clientid_other) and not XQFunction.isStrNil(wifidev_other) then
           local wifinet_other = wifidev_other:get_wifinet(clientid_other)
           if not XQFunction.isStrNil(wifinet_other) then
...
function _wifiScan(ifname)
    local LuciUtil = require("luci.util")
    local result = {}
    local scan = "iwpriv "..ifname.." get_site_survey"
...
function getWifiScanlist(sitesurvey)
    ...
    if wifi_on ~= 1 then
        ...
        -- fix bsd
        local wifi_b = getWifiBasicInfo(1)
        if wifi_b.bsd and tonumber(wifi_b.bsd) == 1 then
            if not XQFunction.isStrNil(WIFI2G) and not XQFunction.isStrNil(WIFI5G) then
                os.execute("uci set wireless."..WIFI2G..".disabled=0;uci set wireless."..WIFI5G..".disabled=0;uci commit wireless;/sbin/wifi >/dev/null 2>/dev/null")
            elseif not XQFunction.isStrNil(WIFI2G) and XQFunction.isStrNil(WIFI5G) then
                os.execute("uci set wireless."..WIFI2G..".disabled=0;uci commit wireless;/sbin/wifi enable "..WIFI2G..">/dev/null 2>/dev/null")
            end
        else
            os.execute("uci set wireless."..WIFI2G..".disabled=0;uci commit wireless;/sbin/wifi enable "..WIFI2G..">/dev/null 2>/dev/null")
    ...
    scan = ""
    if not XQFunction.isStrNil(apclient_5g) then
        scan = "iwpriv "..apclient_5g.." set SiteSurvey=\""..sitesurvey.."\";sleep 1;"
    end
    if not XQFunction.isStrNil(apclient_2g) then
        scan = scan.."iwpriv "..apclient_2g.." set SiteSurvey=\""..sitesurvey.."\";sleep 1;"
    end
    os.execute(scan)
    if not XQFunction.isStrNil(apclient_5g) then
        local scanresult = _wifiScan(apclient_5g)
    ...
    if not XQFunction.isStrNil(apclient_2g) then
        local scanresult_ = _wifiScan(apclient_2g)
        if #scanresult_ > 0 then
            for _, item in ipairs(scanresult_) do
                table.insert(result, item)
            end
        end
    end
...
if HARDWARE:match("^r1c") or HARDWARE:match("^r3") then
...
function iwprivSetChannel(channel1, channel2)
    if channel1 then
        local setcmd = "sleep 4; iwpriv wl1 set Channel=\""..XQFunction._cmdformat(tostring(channel1)).."\""
