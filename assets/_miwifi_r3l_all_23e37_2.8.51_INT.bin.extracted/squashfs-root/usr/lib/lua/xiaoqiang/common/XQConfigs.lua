...
--    2 : preview
SERVER_CONFIG = 0
SERVER_CONFIG_ONLINE_URL = "http://in.api.miwifi.com"
...
LOG_ZIP_FILEPATH = "/tmp/log.tar.gz"
...
FORK_RESTART_WIFI_NOW = "/sbin/wifi >/dev/null 2>/dev/null; /etc/init.d/minidlna restart; /etc/init.d/samba restart; ubus call trafficd reload;/usr/bin/gettraffic flush_wl_dev >/dev/null 2>/dev/null"
...
FORK_RESET_ALL = "env -i sleep 4 && nvram set restore_defaults=1 && nvram set web_restore=1 && nvram commit && reboot & >/dev/null 2>/dev/null"
