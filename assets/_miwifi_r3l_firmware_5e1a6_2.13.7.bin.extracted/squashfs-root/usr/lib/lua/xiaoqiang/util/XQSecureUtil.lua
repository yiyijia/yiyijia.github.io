...
local POLAR_DECCIPERTEXT = "echo -e '%s' | /usr/bin/aes_crypt -b -d -k %s -i 64175472480004614961023454661220 -"
...
function xssCheck(value)
    ...
    if type(value) == "string" then
        ...
        if cvalue then
            return cvalue    -- here, cvalue is replaced-and-corrected-html
        else
            local XQLog = require("xiaoqiang.XQLog")
...
function decCiphertext(user, ciphertext)
    if XQFunction.isStrNil(ciphertext) or
        ciphertext:match("\"") or
        ciphertext:match(" ") or
        ciphertext:match("'")
    then
    ...
    local cmd1 = string.format(POLAR_DECCIPERTEXT, ciphertext, password)
    if os.execute(cmd) == 0 then
        return luci.util.trim(luci.util.exec(cmd))
    elseif os.execute(cmd1) == 0 then
        return luci.util.trim(luci.util.exec(cmd1))
    end
    return nil
end
...
function hackCheck(key,value)
    -- just return for current-version
    if true then
        return value
    end
    ...
    -- return '' if hack-chars found.
    if string.find(value,filterChars) then
        return ''
    end
