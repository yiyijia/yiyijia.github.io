...
function wifinet.set_list(self, opt, val)
    if val and type(val) == "table" and #val > 0 then
        return _uci_real:set_list("wireless", self.sid, opt, val)
    else
        return _uci_real:delete("wireless", self.sid, opt)
    end
end
...
function wifinet.channel(self)
    return self.iwinfo.channel or
        tostring(_uci_state:get("wireless", self.iwdata.device, "channel"))
end

function wifinet.confchannel(self)
    return tostring(_uci_state:get("wireless", self.iwdata.device, "channel"))
end

function wifinet.bw(self)
    return self.iwinfo.bw or
        tostring(_uci_state:get("wireless", self.iwdata.device, "bw"))
end

function wifinet.confbw(self)
    return tostring(_uci_state:get("wireless", self.iwdata.device, "bw"))
end
...
function wifinet.scanlist(self)
