...
function setWifi()
    ...
    local txbf = tonumber(LuciHttp.formvalue("txbf"))
    if on ~= nil then
        on = tonumber(on)
    end
    if channel == "0" then
...
function disableap()
    ...
    local XQFunction = require("xiaoqiang.common.XQFunction")
    local ifname = XQFunction.get_active_apcli()
    local result = {
        ["code"] = 0
    }
...
function getMode()
    ...
    elseif mode == "wifiapmode" then
        ...
        result.hostip = hostip
        result.hostname = hostname
        active_apcli = XQFunction.get_active_apcli()
        local apcli = XQWifiUtil.getWifiBridgedClient(active_apcli)
        if apcli then
...
function setWifiApMode()
    ...
    if ssid then
        local ap = XQAPModule.setWifiAPMode(ssid, password, nssid, nencryption, npassword, false, band)
        if not ap.scan then
            result.code = 1617
