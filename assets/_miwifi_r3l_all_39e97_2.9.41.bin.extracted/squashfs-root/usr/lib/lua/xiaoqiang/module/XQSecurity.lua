...
function security_status()
    ...
    local status = {
        ["wifi_arn"] = 0,
        ["anti_hijack"] = tonumber(uci:get("dnsfixd", "global", "disabled") or 0) == 0 and 1 or 0,
...
function security_switch(values)
    ...
    if values and type(values) == "table" then
        for key, value in pairs(values) do
            ...
            elseif key == "anti_hijack" then
                uci:set("dnsfixd", "global", "disabled", open == 0 and 1 or 0)
                uci:commit("dnsfixd")
                XQFunction.forkExec("/etc/init.d/dnsfixd "..(value == 1 and "on" or "off"))
            end
        end
        uci:commit("security")
        XQFunction.forkExec("touch /etc/config/securitypage/enable.tag;/etc/init.d/securitypage restart")
